...
export const findAndReferLoginCustody = async (page: Page) => {
  ...
  await page
    .getByLabel("Username")
    .fill(process.env.HMPPS_CUSTODY_AUTH_USERNAME as string);
  await page
    .getByLabel("Password")
    .fill(process.env.HMPPS_CUSTODY_AUTH_PASSWORD as string);
...
export const findAndReferLoginCommunity = async (page: Page) => {
  ...
  await page
    .getByLabel("Username")
    .fill(process.env.HMPPS_COMMUNITY_AUTH_USERNAME as string);
  await page
    .getByLabel("Password")
    .fill(process.env.HMPPS_COMMUNITY_AUTH_PASSWORD as string);
  await page.locator("#submit", { hasText: "Sign in" }).click();
...
export const findAndReferUnauthorised = async (page: Page) => {
  ...
  await page
    .getByLabel("Username")
    .fill(process.env.HMPPS_UNAUTHORISED_AUTH_USERNAME as string);
  await page
    .getByLabel("Password")
    .fill(process.env.HMPPS_UNAUTHORISED_AUTH_PASSWORD as string);
  await page.locator("#submit", { hasText: "Sign in" }).click();
